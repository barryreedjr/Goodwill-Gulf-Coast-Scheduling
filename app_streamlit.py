...
def can_assign(emp, role, day_name, start_str, end_str, avail_map, funcs_map, assigned, rules, assigned_hours, days_worked, min_hours_map, max_hours_map):
    ...
    if assigned_hours.get(emp, 0) + sh > max_hours_map.get(emp, float('inf')):
        return False
...
def run_schedule(file, startdate_str=None, role_colors=None):
    ...
    max_hours_map = dict(zip(employees["Employee"], pd.to_numeric(employees.get("MaxHours", 999), errors="coerce").fillna(999)))
    ...
    for w_idx, week_dates in enumerate(weeks):
        ...
        for d_idx, date in enumerate(week_dates):
            ...
            for _, r in day_cov.iterrows():
                ...
                demands += [{"Role": r["Role"], "ShiftType": r["ShiftType"]} for _ in range(count)]
            ...
            for dem in demands:
                ...
                for e in employees["Employee"]:
                    ...
                    if not can_assign(e, role, day_name, s_str, e_str, local_avail, funcs_map, assignments, rules, assigned_hours, days_worked, min_hours_map, max_hours_map):
                        continue
...
with st.sidebar:
    st.header("Template")
    st.download_button(
        label="Download blank template (v3)",
        data=generate_template_bytes(),
        file_name="schedule_input_template_v3.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.caption("Template includes skills matrix (Yes/No), availability_simple (Yes = NOT available), rules Min=3 / Max=10, and anchors (Open 08:00, Mid 10:00, Close 20:30).")
