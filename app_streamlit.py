...
def run_schedule(file, startdate_str=None, role_colors=None):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        # role colors (edit here if you want different colors)
        default_role_colors = {
            "Cashier": "#E3F2FD",   # light blue
            "Donation": "#E8F5E9",  # light green
            "Pricer": "#FFF3E0",    # light orange
            "Hanger": "#F3E5F5",    # light purple
            "Manager": "#FFEBEE",   # light red/pink
        }
        ...
        for wk in range(1,5):
            ...
            for e in emps:
                for d in DAYS:
                    ...
                    if len(rows)==0:
                        frame.loc[frame["Employee"]==e, d] = ("OFF", None)
                        continue
                    # If multiple roles are assigned in same day, join; use first role for color
                    parts = []
                    first_role = None
            ...
            # Temp replace tuples with text for DataFrame write; we'll reformat per-cell
            for d in DAYS:
            ...
            # Print setup
            ws.set_landscape()
            ws.fit_to_pages(1, 0)      # fit to 1 page width
            ws.repeat_rows(1, 1)       # repeat header row on each printed page
            ws.center_horizontally()
            # Apply per-cell role-based colors
