...
def build_availability_map(employees, availability, availability_simple):
    avail = {}
    # 1) availability_simple (Yes = NOT available)
    ...
    if availability is not None and not availability.empty:
        ...
        return avail
    # 3) default: everyone fully available
...
def employee_functions_map(employees):
    # Prefer "skills matrix" columns (each role a Yes/No column). Fallback to CSV "Functions".
    # Normalize "Yes" values and title case.
        ...
        for _, r in employees.iterrows():
            ...
            for role in role_cols_present:
                ...
                if val in ("yes","y","true","1","x"):
                    skills.add(role)
            # Fallback to CSV if no "yes" found AND a Functions column exists
            ...
            m[emp] = skills
        return m
    # No matrix columns; use CSV column
    for _, r in employees.iterrows():
        emp = r["Employee"]
        skills = set()
        raw = str(r.get("Functions",""))
...
# --- Robust Template Download ---
def generate_template_bytes():
    # Employees sheet with skills matrix and simple availability sheet
    employees_df = pd.DataFrame({
        ...
        "Hanger": ["No","No"],
        "Manager": ["Yes","No"],
        # Optional legacy column (not needed if matrix used):
        "Functions": ["Manager,Cashier", "Donation,Pricer"]
    })
